CREATE SINK CONNECTOR `mapping_to_neo4j` WITH(
    ...
    "neo4j.topic.cypher.mapping" = 'MERGE (h:Host{ip: event.ip}) ON CREATE SET h.url = event.url, h.country=event.country, h.city=event.city, h.latitude=event.latitude, h.longitude=event.longitude ON MATCH SET h.url = event.url, h.country=event.country, h.city=event.city, h.latitude=event.latitude, h.longitude=event.longitude'
);